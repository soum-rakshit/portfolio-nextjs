...
export default function ProjectModal({ projects, initialProjectId, onClose }) {
  ...
  return (
    ...
    >
      ...
      {activeIndex < projects.length - 1 && (
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 md:w-7 md:h-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m9 18 6-6-6-6"/></svg>
        </button>
      )}

      <div 
        className={`relative w-full h-full max-h-[90vh] flex items-center justify-center pointer-events-none ${
          isClosing 
      ...
      >
        <div className="w-full h-full overflow-hidden relative pointer-events-auto">
          <div 
            className="flex transition-transform duration-500 absolute h-full items-center"
            style={{
              '--card-width': 'min(800px, calc(100vw - 48px))',
              '--card-gap': '32px',
              left: '50%',
              gap: 'var(--card-gap)',
          ...
          >
            {projects.map((project, idx) => {
              ...
              <div 
                key={project.id}
                className={`h-full max-h-[90vh] flex-shrink-0 bg-[#0b0c14] border rounded-[20px] overflow-hidden flex flex-col transition-all duration-500 ${
                  isActive 
                    ? 'border-emerald-500/60 scale-100 opacity-100 shadow-[0_25px_50px_-12px_rgba(16,185,129,0.25)] pointer-events-auto' 
                    : 'border-white/5 scale-95 opacity-35 pointer-events-auto cursor-pointer'
                }`}
              ...
              >
                <div className="overflow-y-auto flex-1 w-full h-full">
                  <div className="w-full h-[250px] bg-gradient-to-r from-emerald-500/15 to-cyan-500/15 relative flex justify-center items-center overflow-hidden border-b border-white/5 shrink-0">
                  ...
                  <div className="p-8 md:p-10 text-left shrink-0">
